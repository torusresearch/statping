...
<template>
  <div class="row p-2">
    <div
      v-if="loaded && last_failure && failureBefore"
      class="col-12 text-danger font-2 m-0 mb-2"
    >
      <font-awesome-icon
        icon="exclamation"
        class="mr-1 text-danger font-weight-bold"
        size="1x"
      /> Recent Failure<br>
      <span
        class="font-italic font-weight-light text-dim mt-1"
    ...
    >
      <div
        ...
        :key="message.id"
        class="col-12 font-2 m-0 mb-2"
      >
        <font-awesome-icon
          icon="calendar"
          class="mr-1"
          size="1x"
        /> Upcoming Announcement<br>
        <span class="font-italic font-weight-light text-dim mt-1">
          {{ message.description }}
        </span>
    ...
    >
      ...
      >
        ...
        <span class="font-0 text-dim float-right font-weight-light mt-1">
          @ <strong>{{ niceDate(incident.created_at) }}</strong>
        </span>
      </div>
    </div>

    <div
      v-if="success_event && !failureBefore"
      class="col-12 font-2 m-0 mb-2"
    >
      ...
      <span
        v-if="!this.isZero(service.last_error)"
