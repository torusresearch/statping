  ...
  <div>
    <form @submit.prevent="saveNotifier">
      <div class="card contain-card mb-3">
        <div class="card-header text-capitalize">
          {{ notifier.title }}
          <span
            class="switch switch-sm switch-rd-gr float-right"
            @click="enableToggle"
          >
            <input
        ...
        <div class="card-body">
          ...
          <div v-if="notifier.method==='mobile'">
            <div class="form-group row mt-3">
              ...
              <div class="col-sm-8">
                <div class="input-group">
                  <input
                    id="statping_domain"
                    :value="$store.getters.core.domain"
                    type="text"
                    class="form-control"
                    readonly
                  >
                  <div class="input-group-append copy-btn">
                    <button
                      ...
                      type="button"
                      @click.prevent="copy($store.getters.core.domain)"
                    >
                      Copy
                    </button>
                  </div>
            ...
            <div class="form-group row mt-3">
              ...
              <div class="col-sm-8">
                <div class="input-group">
                  <input
                    id="apisecret"
                    :value="$store.getters.core.api_secret"
                    type="text"
                    class="form-control"
                    readonly
                  >
                  <div class="input-group-append copy-btn">
                    <button
                      class="btn btn-outline-secondary"
                      type="button"
                      @click.prevent="copy($store.getters.core.api_secret)"
                    >
                      Copy
                    </button>
                  </div>
            ...
            <div class="col-12 col-md-6 offset-0 offset-md-3">
              <img
                ...
                class="img-thumbnail"
              >
              <span class="text-muted small center">
          ...
          <div 
            v-if="notifier.method!=='mobile'"
          >
            <div
              v-for="(form, index) in notifier.form"
              :key="index"
              class="form-group"
            >
              ...
              >
                <option
                  v-for="(val, k) in form.list_options"
                  :key="k"
                  :value="val"
                >
                  {{ val }}
              ...
              >
                ...
                <label
                  class="mb-0"
                  :for="`switch_${notifier.name}_${form.field}`"
                />
              </span>

              <small
                class="form-text text-muted"
                v-html="form.small_text"
              />
            </div>
          </div>
          ...
          <div class="row mt-4">
            <div class="col-sm-12">
              <span class="slider-info">
                Limit {{ notifier.limits }} per hour
              </span>
              <input
                v-model.number="notifier.limits"
                type="range"
                name="limits"
                class="slider"
                min="1"
              ...
              <small class="form-text text-muted">
                Notifier '{{ notifier.title }}' will send a maximum of {{ notifier.limits }} notifications per hour.
              </small>
            </div>
          </div>
        </div>
      ...
      >
        <div class="card-header text-capitalize">
          ...
          <span class="badge badge-dark float-right text-uppercase mt-1">
            {{ notifier.data_type }}
          </span>
        </div>
        <div
        ...
        >
          <span
            v-if="notifier.request_info"
            class="text-muted d-block mb-3"
            v-html="notifier.request_info"
          />
    ...
    >
      <div class="card-header text-capitalize">
        ...
        />
        {{ notifier.title }} Logs
        <span class="badge badge-info float-right text-uppercase mt-1">
          {{ notifier.logs.length }}
        </span>
      </div>
      <div
        class="card-body"
        :class="{'d-none': !expanded_logs}"
      >
        <div
          v-for="(log, i) in notifier.logs"
          :key="i"
          class="alert"
          :class="{'alert-danger': log.error, 'alert-dark': !log.success && !log.error, 'alert-success': log.success && !log.error}"
        >
          <span class="d-block">
            Service {{ log.service }}
          ...
          <div
            v-if="log.message !== ''"
            class="bg-white p-3 small mt-2"
          >
            <code>{{ log.message }}</code>
          </div>

          <div class="row mt-2">
            <span class="col-6 small">
    ...
    <span class="d-block small text-center mb-3">
      <span class="text-capitalize">
        {{ notifier.title }}
      </span> Notifier created by <a
        :href="notifier.author_url"
        target="_blank"
      >
        {{ notifier.author }}
      </a>
